...
game* new_game(unsigned int run, unsigned int width, unsigned int height,
    enum type type)
{
    if (width <= 1 || height <= 1 || run <= 1) {
        printf("\n");
        fprintf(stderr, 
            "width, height and/or run invalid, unspecified or less than 2");
        printf("\n\n");
        exit(1);
     }
     ...
     if (run > width || run > height) {
        printf("\n");
        fprintf(stderr, "run must be smaller than or equal to width or height");
        printf("\n\n");
        exit(1);
    }
